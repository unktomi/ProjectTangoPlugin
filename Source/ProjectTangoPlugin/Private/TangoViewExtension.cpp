...
bool FTangoViewExtension::IdentifyViewWithCameraComponent(const FSceneView* InView,int32 Stride)
{
	...
	for (int32 i = 0; i < Cameras[Stride].Num(); ++i)
	{
		...
		{
			UE_LOG(ProjectTangoPlugin, Log, TEXT("Actorname: %s compared to: %s and there also is: %s"), *InView->ViewActor->GetName(), *CameraActors[i]->GetName(),*ARComponent->GetActor()->GetName());
		}*/
		if (Cameras[Stride][i].Actor == InView->ViewActor)
		{
			//UE_LOG(ProjectTangoPlugin, Log, TEXT("FTangoViewExtension::IdentifyViewWithCameraComponent: identified Actor"));
			...
			{
				//UE_LOG(ProjectTangoPlugin, Log, TEXT("FTangoViewExtension::IdentifyViewWithCameraComponent: identified position"));
				return true;
			}
		}
	}
	UE_LOG(ProjectTangoPlugin, Warning, TEXT("FTangoViewExtension::IdentifyViewWithCameraComponent: Did not identify Actor"));
	return false;
}
...
const bool FTangoViewExtension::GetLateUpdateTransform(FTransform& Transform,int32 Stride, bool bIsAbsolute)
{
	...
	UE_LOG(ProjectTangoPlugin, Warning, TEXT("FTangoViewExtension::GetLateUpdateTransform: Failed because pose is invalid!"));
	Transform = FTransform::Identity;
	return false;
}
...
void FTangoViewExtension::PreRenderViewFamily_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneViewFamily& InViewFamily)
{
	...
	{
		UE_LOG(ProjectTangoPlugin, Warning, TEXT("FTangoViewExtension::PreRenderViewFamily_RenderThread: No Stride found!"))
	}
}
...
void FTangoViewExtension::PreRenderView_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneView& InView)
{
	...
	if (InView.Family)
	{
		...
		{
			UE_LOG(ProjectTangoPlugin, Warning, TEXT("FTangoViewExtension::PreRenderView_RenderThread: No Stride could be found!"));
			InView.ProjectionMatrixUnadjustedForRHI = TangoARHelpers::GetARProjectionMatrix();
			InView.ViewMatrices.ProjMatrix = AdjustProjectionMatrixForRHI(InView.ProjectionMatrixUnadjustedForRHI);
	...
	{
		UE_LOG(ProjectTangoPlugin, Warning, TEXT("FTangoViewExtension::PreRenderView_RenderThread: InView.Family is nullptr!"));
		InView.ProjectionMatrixUnadjustedForRHI = TangoARHelpers::GetARProjectionMatrix();
		InView.ViewMatrices.ProjMatrix = AdjustProjectionMatrixForRHI(InView.ProjectionMatrixUnadjustedForRHI);
		InView.UpdateViewMatrix();
	}
...
void FTangoViewExtension::BeginRenderViewFamily(FSceneViewFamily& InViewFamily)
{
	...
	//UE_LOG(ProjectTangoPlugin, Log, TEXT("FTangoViewExtension::BeginRenderViewFamily: Called"));
	...
	//UE_LOG(ProjectTangoPlugin, Log, TEXT("FTangoViewExtension::BeginRenderViewFamily: Found %d Sceneproxies and %d Cameras!"), LateUpdateSceneProxyCount, CameraCount);
}
